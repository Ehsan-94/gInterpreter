...
class SA_GC(object):
    def __init__(self, task, method, model_name, graph, importance_threshold, load_index, input_dim, hid_dim,
                 output_dim, normalize_coeff):
        self.normalize_coeff = normalize_coeff
        self.GCN_model = self.load_model(task, method, model_name, load_index=load_index, input_dim=input_dim, hid_dim=hid_dim,
    ...
    def compute_square_gradients(self, model, dataset):
        ...
        #print("per_sample_grads_wrt_graph_label: ", per_sample_grads_wrt_graph_label)

        grads_wrt_graph_label = torch.square(per_sample_grads_wrt_graph_label[1])
        ...
        grads_wrt_class_zero = torch.square(per_sample_grads_wrt_class_zero[1])
        ...
        grads_wrt_class_one = torch.square(per_sample_grads_wrt_class_one[1])
        ...
        square_grads_wrt_graph_label = self.column_wise_addups(square_grads_wrt_graph_label)
        square_grads_wrt_class_zero = self.column_wise_addups(square_grads_wrt_class_zero)
        square_grads_wrt_class_one = self.column_wise_addups(square_grads_wrt_class_one)
    ...
    def drop_important_nodes(self, input_graphs, importance_threshold):
        square_grads_wrt_graph_label, square_grads_wrt_class_zero, square_grads_wrt_class_one = self.compute_square_gradients(
          self.GCN_model, input_graphs)

        SA_attribution_scores = self.saliency(input_graphs, square_grads_wrt_graph_label)

        occluded_GNNgraph_list = []

        for i in range(len(SA_attribution_scores)):
            sample_graph = deepcopy(input_graphs[i])
            graph_dict = {}
            for j in range(len(sample_graph.x)):

                if self.is_salient(SA_attribution_scores[i][j], importance_threshold):
        ...
        #Standard_SA_attribution_scores = self.standardize_by_softmax(SA_attribution_scores)
        return occluded_GNNgraph_list, SA_attribution_scores


#dataset = TUDataset(root='data/TUDataset', name='MUTAG')

#new_output = SA_GC(task="Graph Classification", method="SA", model_name="GCN_plus_GAP", graph=[dataset[0]], importance_threshold=0.5, load_index=100,
#                   input_dim=len(dataset[0].x[0]), hid_dim=7, output_dim=2, normalize_coeff=100)
